...
import { JOURNAL_EDITOR_BOUNDARY, journalEditorBoundaryPlugin, withoutJournalEditorBoundary } from "./scripts/journal-editor-boundary.mjs";
...
export const blockedFileSystemPlugin = {
  ...
  setup(build) {
    build.onResolve(
      ...
      (args) => args.importer.replaceAll("\\", "/").endsWith("/src/journal/systemFonts.ts") && args.path === "node:fs/promises" ? { path: args.path, external: true } : ({
        path: args.path,
...
const verifyBundledCodePlugin = {
  ...
  setup(build) {
    build.onEnd(async (result) => {
      ...
      if(result.metafile){await fsp.mkdir('test-artifacts',{recursive:true});await fsp.writeFile('test-artifacts/bundle-metafile.json',JSON.stringify(result.metafile));}
      const findings = findUnsafeRuntimeConstructs(withoutJournalEditorBoundary(code));
      if (findings.length > 0) {
        throw new Error(
...
const options = {
  banner: {
    ...
    js:
      "/* THIS IS A GENERATED/BUNDLED FILE BY ESBUILD */\n" + JOURNAL_EDITOR_BOUNDARY + "\n" +
      "/*! Academic Editor Ultra source editor dependencies\n" + journalEditorLicenses.replaceAll('*/','* /') + "\n*/\n" +
      "/*! HanMark local OCR: Tesseract.js 7.0.0, Tesseract.js-core 7.0.0 and tessdata_fast eng/kor. HanMark supplies local verified bytes and disables network resource loading.\n" + journalOcrLicense.replaceAll('*/','* /') + "\n*/\n" +
  ...
  plugins: [
    // Private equivalence builds use preserved source without replacing the
    // working tree. This plugin is inert for ordinary development/releases.
    {name:"saved-journal-baseline",setup(build){if(process.env.HANMARK_BASELINE_DIR)build.onLoad({filter:/[\\/]src[\\/]journal[\\/](layout|typography|engine|editableExport|embedded\.generated)\.ts$/},async args=>({contents:await fsp.readFile(nodePath.join(process.env.HANMARK_BASELINE_DIR,nodePath.relative(process.cwd(),args.path)),"utf8"),loader:"ts",resolveDir:nodePath.dirname(args.path)}));}},
    blockedFileSystemPlugin,
    journalEditorBoundaryPlugin,
