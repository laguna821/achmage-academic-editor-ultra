...
import { withoutJournalEditorBoundary } from "./journal-editor-boundary.mjs";
...
const bundle = withoutJournalEditorBoundary(await read("main.js"));
...
for (const [path, content] of sourceEntries) {
  ...
  if(normalizedPath !== "src/journal/systemFonts.ts") check(
    path,
    content,
  ...
  if (!["src/legacy-port/userProcess.ts","src/journal/systemFonts.ts"].includes(normalizedPath)) {
    check(
      path,
      content,
...
check(
  "main.js",
  bundle.replace(/require\("node:fs\/promises"\)/u,""),
...
if (processBoundaries.length !== 1) {
  findings.push(
    `main.js: expected only the read-only font boundary, found ${processBoundaries.length}`
  );
}
const fontAdapter=await read("src/journal/systemFonts.ts");
if(/\b(?:writeFile|unlink|rename|mkdir|rmdir|appendFile|execSync|spawn|exec)\s*\(/u.test(fontAdapter)||!fontAdapter.includes("shell:false")||!fontAdapter.includes("allowed.get(id)"))findings.push("Read-only font boundary changed; review its capabilities.");
if((bundle.match(/require\("node:fs\/promises"\)/gu)??[]).length!==1)findings.push("Expected exactly one read-only font filesystem boundary");
