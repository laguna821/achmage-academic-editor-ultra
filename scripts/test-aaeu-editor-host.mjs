...
try {
  ...
  const templates=await page.evaluate(async()=>{const lib=__view.library(),all=await lib.list(),custom=structuredClone(all.find(t=>t.id==='builtin:general'));custom.id='custom:editor-test';custom.name='Editor test';await lib.save(custom);return ['builtin:achmage','builtin:command-space','custom:editor-test','builtin:hnmr'];});
  ...
  // AF embeds exactly the original HNMR PDF, with separate display caches and hashes.
  ...
  const logo=report.originalAssets.find(a=>a.sha256==='e746e6a8b9420ed99116dc2142be80724b876665a7f2f514b025fbc73e7b4b2d');assert.ok(logo,JSON.stringify(report.originalAssets));
