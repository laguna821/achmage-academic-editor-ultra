...
export default class AcademicEditorUltra extends Plugin {
  async onload():Promise<void>{
    ...
    this.addCommand({id:'open-sample-manuscript',name:t('설명서 원고로 시작'),callback:()=>run(async()=>{await (await open()).openSampleManuscript();})});
    this.addCommand({id:'open-journal-editor',name:t('저널 편집 열기'),callback:()=>run(open)});
    this.addCommand({id:'new-journal-markdown',name:t('저널 Markdown 원고 만들기'),callback:()=>run(async()=>{await (await open()).createMarkdownManuscript();})});
    this.addCommand({id:'markdown-to-journal',name:t('현재 Markdown 원문으로 저널 시작'),checkCallback:checking=>{const current=source();if(!current)return false;if(!checking)run(async()=>{await (await open()).importMarkdownNote(current.file,current.text);});return true;}});
    this.addCommand({id:'migrate-hanmark-journals',name:t('HanMark 저널 프로젝트 복사 가져오기'),callback:()=>run(async()=>{await (await open()).migrateHanmark();})});
