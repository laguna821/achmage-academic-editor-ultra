...
/** Journal imports retain source ranges while sharing the existing Markdown grammar. */
export function parseEditorialSourceBlocks(markdown:string):{block:EditorialBlock;startLine:number;endLine:number}[]{
  const tokens=MARKDOWN.parse(markdown,{});
  // The export parser intentionally accepts embedded raster URLs only. Journal
  // importing must retain unresolved local/SVG URLs for its snapshot loader.
  // Temporary inert tokens never leave this function or reach a renderer.
  const images=new Map<string,string>();
  const placeholder=(src:string):string=>{const key=`data:image/png;base64,iVBORw0KGgoA${(images.size+1).toString(16).padStart(8,'0')}AAAA`;images.set(key,src);return key;};
  const prepare=(list:MarkdownToken[]):MarkdownToken[]=>list.flatMap(t=>{
    if(t.children)t.children=prepare(t.children);
    if(t.type==='image'){const src=t.attrs?.find(a=>a[0]==='src');if(src)src[1]=placeholder(src[1]);}
    if(t.type==='html_inline'||t.type==='html_block')t.content=t.content.replace(/(<img\b[^>]*?\bsrc\s*=\s*)(["'])(.*?)\2/gi,(_m:string,prefix:string,quote:string,src:string)=>prefix+quote+placeholder(src)+quote);
    if(t.type!=='text')return [t];
    const out:MarkdownToken[]=[];let at=0;
    for(const m of t.content.matchAll(/!\[\[([^\]\r\n]+)\]\]/g)){
      const [src,alias]=m[1].split('|');if(!/\.(png|jpe?g|gif|bmp|svg|webp|tiff?|emf|wmf|pdf)(?:[?#].*)?$/i.test(src.trim()))continue;
      if(m.index>at)out.push({...t,content:t.content.slice(at,m.index)});
      const alt=alias&&!/^\d+(?:x\d+)?$/.test(alias)?alias:src.split('/').at(-1)??'';
      out.push({...t,type:'image',tag:'img',content:alt,attrs:[['src',placeholder(src.trim())],['alt',alt]],children:null});at=m.index+m[0].length;
    }
    if(!at)return [t];if(at<t.content.length)out.push({...t,content:t.content.slice(at)});return out;
  });
  const restore=(value:unknown):void=>{if(!value||typeof value!=='object')return;const object=value as Record<string,unknown>;if(object.type==='image'&&typeof object.src==='string'&&images.has(object.src))object.src=images.get(object.src);for(const child of Object.values(object))restore(child);};
  const result:{block:EditorialBlock;startLine:number;endLine:number}[]=[];
  for(let i=0;i<tokens.length;){
    const start=i;let depth=tokens[i].nesting;i++;
    while(depth>0&&i<tokens.length){depth+=tokens[i].nesting;i++;}
    const range=tokens[start].map??[0,markdown.split('\n').length];
    const group=prepare(tokens.slice(start,i));
    for(const block of parseMarkdownBlocks(group,{index:0})){restore(block);result.push({block,startLine:range[0]+1,endLine:range[1]});}
  }
  return result;
}
