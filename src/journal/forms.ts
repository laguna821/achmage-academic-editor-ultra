...
export function textDialog(app:App,title:string,label:string,value:string,multiline=false):Promise<string|null>{
  return new Promise(resolve=>{
    ...
    action(modal.contentEl,"확인",()=>{result=input.value;modal.close();});action(modal.contentEl,"취소",()=>modal.close());
...
export function referenceDialog(app:App,reference:ReferenceRecord):Promise<ReferenceRecord|null>{
  return new Promise(resolve=>{
    ...
    modal.titleEl.setText("참고문헌 서지정보");
    if(r.raw)modal.contentEl.createEl("p",{text:r.raw,cls:"aaeu-journal-original"});
    choose(modal.contentEl,"자료 유형",r.type,{"article-journal":"학술지 논문",book:"책",chapter:"편저서 장",report:"보고서",webpage:"웹페이지",thesis:"학위논문","paper-conference":"학술대회",dataset:"데이터",software:"소프트웨어",unknown:"미확인"},v=>r.type=v as ReferenceType);
    ...
    const labels:Partial<Record<keyof ReferenceRecord,string>>={title:"제목 · 고유명사를 보존한 문장식 대소문자",year:"연도",containerTitle:"학술지·책·사이트명",volume:"권",issue:"호",pages:"쪽",publisher:"출판사",edition:"판",doi:"DOI",url:"URL",accessed:"조회 날짜 (YYYY-MM-DD)",number:"보고서 번호",genre:"자료 설명",institution:"기관",version:"버전"};
    ...
    choose(modal.contentEl,"날짜 없는 자료",r.knownNoDate?"yes":"no",{no:"아니요",yes:"실제 무연도 자료"},v=>r.knownNoDate=v==="yes");
    choose(modal.contentEl,"내용 변경 가능",r.changing?"yes":"no",{no:"고정된 자료",yes:"계속 변경됨"},v=>r.changing=v==="yes");
    action(modal.contentEl,"서지정보 확정",()=>{r.confirmed=true;r.provenance.push({source:"Editor confirmation",date:new Date().toISOString(),fields:Object.keys(r)});result=r;modal.close();});
    action(modal.contentEl,"미확정으로 저장",()=>{r.confirmed=false;result=r;modal.close();});action(modal.contentEl,"취소",()=>modal.close());
