...
export function referenceDialog(app:App,reference:ReferenceRecord):Promise<ReferenceRecord|null>{
  return new Promise(resolve=>{
    ...
    field(modal.contentEl,localizeUi("저자 · 한 줄에 성, 이름 또는 단체명"),r.author.map(a=>a.literal??`${a.family??""}, ${a.given??""}`).join("\n"),v=>r.author=v.split("\n").filter(v=>v.trim()).map(splitAuthor),true);
    const labels:Partial<Record<keyof ReferenceRecord,string>>={title:localizeUi("제목 · 고유명사를 보존한 문장식 대소문자"),year:t("연도"),containerTitle:localizeUi("학술지·책·사이트명"),volume:t("권"),issue:t("호"),pages:t("쪽"),publisher:localizeUi("출판사"),edition:localizeUi("판"),doi:"DOI",url:"URL",accessed:localizeUi("조회 날짜 (YYYY-MM-DD)"),number:localizeUi("보고서 번호"),genre:localizeUi("자료 설명"),institution:localizeUi("기관"),version:localizeUi("버전")};
    ...
    field(modal.contentEl,localizeUi("편집자 · 한 줄에 성, 이름"),r.editors?.map(a=>a.literal??`${a.family??""}, ${a.given??""}`).join("\n")??"",v=>r.editors=v.split("\n").filter(v=>v.trim()).map(splitAuthor),true);
    choose(modal.contentEl,localizeUi("날짜 없는 자료"),r.knownNoDate?"yes":"no",{no:t("아니요"),yes:t("실제 무연도 자료")},v=>r.knownNoDate=v==="yes");
    choose(modal.contentEl,localizeUi("내용 변경 가능"),r.changing?"yes":"no",{no:t("고정된 자료"),yes:t("계속 변경됨")},v=>r.changing=v==="yes");
