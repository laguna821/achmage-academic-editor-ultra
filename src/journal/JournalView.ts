import {t as localizeUi} from './i18n';
import {prepareBrandAssets} from './brandAssets';
import {t} from './i18n';
import {localizedProperty} from './propertyLanguage';
import {ManuscriptEditor} from './manuscriptEditor';
import {patchSourceProperties,mergeSourceEdits,SourceEditConflict,sourceProperties} from './sourceEditing';
import {fieldForSelection,updateWordProperties} from './manuscriptFields';
...
import {applyTemplate,copyTemplateAssets,restoreLegacyTemplateAssets,exportTemplate,templateFromProject,createPublicJournalProject} from './templates';
...
import { JournalHistory,  setChangeDecision, validateProject } from "./project";
...
export class JournalView extends ItemView{
  private history=new JournalHistory(createPublicJournalProject());
  ...
  private manuscript:ManuscriptEditor|null=null;
  private manuscriptKey='';
  private sourceBase:string|null=null;
  private sourceDraft:string|null=null;
  private draftTimer:number|null=null;
  private draftTask:Promise<void>|null=null;
  private sourceUndo:string[]=[];
  private sourceUndoIndex=0;
  private sourceEditAt=0;
  private sourceConflict=false;
  private conflictContext:{error:SourceEditConflict;file:TFile}|null=null;
  private autoPreview=false;
  private previewDirty=false;
  private previewState:HTMLElement|null=null;
  private autoPreviewInput:HTMLInputElement|null=null;
  private quickTemplate:HTMLSelectElement|null=null;
  ...
  getState():Record<string,unknown>{return {root:this.store.root,autoPreview:this.autoPreview};}
  async setState(state:Record<string,unknown>,result:ViewStateResult):Promise<void>{
    this.autoPreview=state.autoPreview===true;if(this.autoPreviewInput)this.autoPreviewInput.checked=this.autoPreview;
    if(typeof state.root==="string"&&state.root!==this.store.root){
      const store=new JournalStore(this.app.vault.adapter,state.root);
      try{const project=await store.load();if(this.sourceDraft!==null)await this.save();this.history=new JournalHistory(project);this.store=store;this.refresh();}catch(e){this.report(e);}
    }
  ...
  async onOpen():Promise<void>{
    ...
    task(t("새 Word 편집본"),async()=>{await this.save();this.history=new JournalHistory(createPublicJournalProject());this.store=new JournalStore(this.app.vault.adapter,PROJECTS+"/"+this.history.current.id);this.startDismissed=false;this.panel='article';this.result=null;this.preview?.clear();this.refresh();await this.save();});
    ...
    action(toolbar,t("템플릿 만들기·수정"),()=>this.openTemplates());
    task(t('템플릿 ZIP 가져오기'),async()=>{const files=await this.gateway.pickFiles({extensions:['zip'],maxFiles:1,maxTotalBytes:40*1024*1024});if(!files[0])return;const template=await this.library().importPackage(files[0].bytes);await this.refreshTemplateChoices();await this.applySavedTemplate(template.id);});
    task(t('현재 템플릿 ZIP 내보내기'),async()=>{await restoreLegacyTemplateAssets(this.project,this.library(),this.store);await prepareBrandAssets(this.project,this.store);const template=templateFromProject(this.project);await this.gateway.saveFile(await exportTemplate(template,this.store),template.name.replace(/[<>:"/\\|?*]/g,'-')+'.journal-template.zip');});
    task(t("글꼴 파일 등록"),()=>this.importFonts());
    task(t("글꼴 목록 새로고침"),async()=>{const c=await discoverSystemFonts(true);this.status.setText(localizeUi("설치 글꼴 {v0}개 · {v1}",{v0:new Set(c.fonts.map(f=>f.family)).size,v1:c.warnings.join(" ")}));});
    task(t("저장"),async()=>{await this.flushSourceDraft();await this.refreshBoundSource();await this.save();});
    ...
    task(uiLanguage()==='ko'?localizeUi('↻ 미리보기 갱신'):'↻ Refresh preview',()=>this.compose());
    this.previewState=toolbar.createSpan({cls:'aaeu-preview-state',attr:{role:'status','aria-live':'polite'}});
    const autoLabel=toolbar.createEl('label',{cls:'aaeu-auto-preview'});
    this.autoPreviewInput=autoLabel.createEl('input',{type:'checkbox'});this.autoPreviewInput.checked=this.autoPreview;
    autoLabel.createSpan({text:uiLanguage()==='ko'?localizeUi('자동 갱신'):'Auto refresh'});autoLabel.title=uiLanguage()==='ko'?localizeUi('선택하면 입력을 멈춘 뒤 2초 후 미리보기를 갱신합니다. 원고는 항상 자동 저장됩니다.'):'Refresh after two seconds of idle time. Manuscript saving is always automatic.';
    this.autoPreviewInput.onchange=()=>{this.autoPreview=this.autoPreviewInput!.checked;this.app.workspace.requestSaveLayout();if(this.renderTimer)this.contentEl.win.clearTimeout(this.renderTimer);this.renderTimer=null;if(this.previewDirty)this.queuePreview();};
    action(toolbar,t("발행정보 확인"),()=>{this.panel="publication";this.manuscript?.focusProperty("aaeu-year");this.renderInspector();});
    ...
    const proofView=action(toolbar,uiLanguage()==='ko'?localizeUi('지면 크게 보기'):'Expand proof',()=>{
      const expanded=this.contentEl.classList.toggle('aaeu-proof-expanded');
      proofView.setAttribute('aria-pressed',String(expanded));
      proofView.setText(uiLanguage()==='ko'?(expanded?localizeUi('편집 패널 보기'):localizeUi('지면 크게 보기')):(expanded?'Show editing panels':'Expand proof'));
    });proofView.setAttribute('aria-pressed','false');
    const group=(label:string,labels:string[]):void=>{
      const menu=toolbar.createEl('details',{cls:'aaeu-toolbar-menu'});menu.createEl('summary',{text:label});const list=menu.createDiv({cls:'aaeu-toolbar-menu-items'});
      for(const button of Array.from(toolbar.querySelectorAll(':scope > button')))if(labels.includes(button.textContent??'')){list.append(button);button.addEventListener('click',()=>{menu.open=false;});}
    };
    group(t('원고'),[t('설명서 원고로 시작'),t('새 Markdown 원고'),t('새 Word 편집본'),t('프로젝트 열기'),t('DOCX·그림 가져오기'),t('Markdown 가져오기'),t('ZIP 열기')]);
    this.quickTemplate=choose(toolbar,t('템플릿'),'',{},id=>{void this.run(()=>this.applySavedTemplate(id));});
    group(t('템플릿 관리'),[t('템플릿 만들기·수정'),t('템플릿 ZIP 가져오기'),t('현재 템플릿 ZIP 내보내기')]);
    group(t('내보내기'),[t('검토용 PDF'),t('최종 PDF'),t('IDML로 내보내기'),t('AF로 내보내기'),t('프로젝트 ZIP')]);
    group(t('더보기'),[t('HanMark 프로젝트 복사'),t('원문 열기'),t('기존 편집본을 원문 모드로 복사'),t('원격 이미지 새로고침'),t('Markdown 원문 다시 가져오기'),t('이미지 캡션 검사'),t('글꼴 파일 등록'),t('글꼴 목록 새로고침')]);
    ...
    this.preview=new JournalPreview(center,(id,navigate)=>this.selectNode(id,navigate),o=>{void this.run(()=>this.applyOverride(o));});
    ...
    const stale=():void=>{if(this.project.markdown){this.sourceStale=true;this.markPreviewDirty();this.status.setText(t('원문 또는 이미지가 변경되었습니다 · 조판/내보내기에서 최신 내용 확인'));}};
  ...
  private report(e:unknown):void{const message=t(e instanceof Error?e.message:String(e));if(this.status)this.status.setText(message);new Notice(message,8000);}
  ...
  private refresh():void{
    ...
    const identity=this.project.id+':'+(this.project.markdown?.path??'word');
    if(identity!==this.manuscriptKey){
      this.editor?.destroy();this.editor=null;this.manuscript?.destroy();this.editorHost.empty();
      this.manuscriptKey=identity;this.editorKey='';this.sourceBase=this.sourceDraft=null;this.sourceUndo=[];this.sourceUndoIndex=0;this.placementUndo=[];this.placementRedo=[];this.sourceConflict=false;
      this.manuscript=new ManuscriptEditor(this.editorHost,this.project,{properties:changes=>{void this.run(()=>this.changeFields(changes));},text:text=>this.changeSource(text),undo:()=>this.undo(),redo:()=>this.redo(),templates:()=>this.openTemplates(),editorEvent:(kind,event,editor)=>{const file=this.app.vault.getAbstractFileByPath(this.project.markdown?.path??'');if(file instanceof TFile)this.app.workspace.trigger(kind,event,editor,{app:this.app,file,editor,hoverPopover:null});}});
      if(this.project.markdown)void this.run(async()=>{
        const file=this.app.vault.getAbstractFileByPath(this.project.markdown!.path);if(!(file instanceof TFile))return;
        const text=await this.sourceText(file);if(this.manuscriptKey!==identity||this.closed)return;
        this.sourceBase=this.sourceDraft=text;this.sourceUndo=[text];this.manuscript?.setSource(text);void this.run(()=>this.checkTemplateMismatch(text));
      });
    }
    this.manuscript?.update(this.project,this.result??undefined);
    if(this.project.markdown){if(this.sourceDraft!==null)this.manuscript?.setSource(this.sourceDraft);}
    else{
      const key=this.project.id+JSON.stringify(this.project.document.blocks);
      if(!this.editor||key!==this.editorKey){
        this.editor?.destroy();this.manuscript!.bodyHost.empty();this.editorKey=key;
        this.editor=new JournalTextEditor(this.manuscript!.bodyHost,this.project,blocks=>{this.edit(p=>{p.document.blocks=blocks;},false);this.editorKey=this.project.id+JSON.stringify(this.project.document.blocks);},id=>{this.selected=id;},()=>this.undo(),()=>this.redo());
      }
    }
    this.refreshStart();this.renderInspector();void this.refreshTemplateChoices();
  }
  private async changeFields(changes:Record<string,unknown>):Promise<void>{
    if(this.project.markdown){
      if(this.sourceDraft===null)throw Error(localizeUi('원문을 불러오는 중입니다.'));
      this.changeSource(patchSourceProperties(this.sourceDraft,changes));
    }else {this.edit(p=>updateWordProperties(p,changes),false);this.manuscript?.update(this.project,this.result??undefined);}
  }
  private changeSource(text:string,record=true):void{
    if(!this.project.markdown||text===this.sourceDraft)return;
    if(record){
      const now=Date.now();this.sourceUndo=this.sourceUndo.slice(0,this.sourceUndoIndex+1);
      if(now-this.sourceEditAt<500&&this.sourceUndoIndex>0)this.sourceUndo[this.sourceUndoIndex]=text;
      else{this.sourceUndo.push(text);this.sourceUndoIndex++;if(this.sourceUndo.length>100){this.sourceUndo.shift();this.sourceUndoIndex--;}}
      this.sourceEditAt=now;
    }
    this.sourceDraft=text;this.sourceStale=true;this.manuscript?.setSource(text);
    // Guided form values follow the draft immediately; composition still imports the actual source.
    this.manuscript?.update(this.project,this.result??undefined);
    this.status.setText(localizeUi('원문 수정 중 · 자동 저장'));
    if(this.draftTimer)this.contentEl.win.clearTimeout(this.draftTimer);
    this.draftTimer=this.contentEl.win.setTimeout(()=>{void this.run(()=>this.flushSourceDraft());},500);
    this.markPreviewDirty();this.queuePreview();
  }
  private flushSourceDraft():Promise<void>{
    if(this.draftTask)return this.draftTask.then(()=>this.flushSourceDraft());
    if(this.sourceConflict){if(this.conflictContext&&!this.contentEl.doc.querySelector('.aaeu-source-conflict'))void this.showSourceConflict(this.conflictContext.error,this.conflictContext.file,false);return Promise.reject(Error(localizeUi('원문 충돌을 먼저 확인하세요. 편집 내용은 보존되어 있습니다.')));}
    if(this.sourceDraft===null||this.sourceBase===null||this.sourceDraft===this.sourceBase)return Promise.resolve();
    this.draftTask=this.writeSourceDraft().finally(()=>{this.draftTask=null;});return this.draftTask;
  }
  private async writeSourceDraft():Promise<void>{
    if(this.draftTimer){this.contentEl.win.clearTimeout(this.draftTimer);this.draftTimer=null;}
    const base=this.sourceBase!,draft=this.sourceDraft!,path=this.project.markdown!.path,key=this.manuscriptKey;
    const file=this.app.vault.getAbstractFileByPath(path);if(!(file instanceof TFile))throw Error(localizeUi('연결된 원문을 찾을 수 없습니다. 편집 내용은 아직 이 창에 있습니다.'));
    try{
      const remote=await this.sourceText(file),disk=await this.app.vault.read(file),merged=mergeSourceEdits(base,draft,remote);
      const views=this.app.workspace.getLeavesOfType('markdown').map(l=>l.view).filter((v):v is MarkdownView=>v instanceof MarkdownView&&v.file?.path===path);
      if(views.some(v=>v.editor.getValue()!==remote))throw new SourceEditConflict(base,draft,views.find(v=>v.editor.getValue()!==remote)!.editor.getValue());
      let written=merged;
      await this.app.vault.process(file,current=>{written=mergeSourceEdits(disk,merged,current);return written;});
      for(const view of views){if(view.editor.getValue()!==remote)throw new SourceEditConflict(base,draft,view.editor.getValue());view.editor.transaction({changes:[{from:{line:0,ch:0},to:view.editor.offsetToPos(remote.length),text:written}]});}
      if(key!==this.manuscriptKey)return;
      const next=this.sourceDraft===draft?written:mergeSourceEdits(draft,this.sourceDraft!,written);
      this.sourceBase=written;this.sourceDraft=next;this.manuscript?.setSource(next);this.status.setText(localizeUi('원문에 저장됨 · ')+path);
    }catch(error){
      if(error instanceof SourceEditConflict){this.sourceConflict=true;await this.showSourceConflict(error,file);}throw error;
    }
  }
  private async showSourceConflict(error:SourceEditConflict,file:TFile,preserve=true):Promise<void>{
    this.conflictContext={error,file};
    const copy=file.path.replace(/\.md$/i,`-conflict-${Date.now()}.md`);if(preserve)await this.app.vault.create(copy,error.local);
    const modal=new Modal(this.app);modal.contentEl.addClass('aaeu-source-conflict');modal.titleEl.setText(localizeUi('원문 변경 충돌 · 편집 사본 보존'));modal.contentEl.createEl('p',{text:localizeUi('내 수정은 충돌 사본으로 보존했습니다. 아래에서 내용을 합친 뒤 적용할 수 있습니다.')});
    const remote=field(modal.contentEl,localizeUi('외부에서 변경된 원문'),error.remote,()=>undefined,true);remote.readOnly=true;
    const merged=field(modal.contentEl,localizeUi('적용할 원문 · 직접 병합'),error.local,()=>undefined,true);
    action(modal.contentEl,localizeUi('병합 내용 적용'),()=>this.run(async()=>{const latest=await this.sourceText(file);if(latest!==error.remote)throw Error(localizeUi('외부 원문이 다시 변경됐습니다. 사본과 최신 원문을 비교하세요.'));this.sourceConflict=false;this.sourceBase=latest;this.sourceDraft=merged.value;await this.flushSourceDraft();modal.close();await this.compose();}));
    action(modal.contentEl,localizeUi('외부 원문 사용 · 내 사본 유지'),()=>this.run(async()=>{const latest=await this.sourceText(file);this.sourceConflict=false;this.sourceBase=this.sourceDraft=latest;this.sourceUndo=[latest];this.sourceUndoIndex=0;this.manuscript?.setSource(latest);modal.close();await this.compose();}));modal.open();
  }
  refreshStart():void{
  ...
  private undo():void{if(this.undoPlacement())return;if(this.project.markdown){if(this.sourceUndoIndex>0){this.sourceEditAt=0;this.changeSource(this.sourceUndo[--this.sourceUndoIndex],false);}return;}this.history.undo();this.dirty=true;this.refresh();void this.run(async()=>{await this.save();this.markPreviewDirty();this.queuePreview();});}
  private redo():void{if(this.undoPlacement(true))return;if(this.project.markdown){if(this.sourceUndoIndex<this.sourceUndo.length-1){this.sourceEditAt=0;this.changeSource(this.sourceUndo[++this.sourceUndoIndex],false);}return;}this.history.redo();this.dirty=true;this.refresh();void this.run(async()=>{await this.save();this.markPreviewDirty();this.queuePreview();});}
  private markPreviewDirty():void{this.previewDirty=true;this.previewState?.setText(uiLanguage()==='ko'?localizeUi('수정사항 있음'):'Changes pending');}
  private queuePreview():void{
    if(this.renderTimer)this.contentEl.win.clearTimeout(this.renderTimer);this.renderTimer=null;
    if(this.autoPreview&&!this.busy)this.renderTimer=this.contentEl.win.setTimeout(()=>{void this.run(()=>this.compose());},2000);
  }
  private async save():Promise<void>{await this.flushSourceDraft();if(this.saveTimer)this.contentEl.win.clearTimeout(this.saveTimer);await this.store.save(this.project);this.dirty=false;if(this.status)this.status.setText(t("저장됨 · ")+this.store.root);}
  ...
  private async importFiles():Promise<void>{
    if(this.project.markdown)throw new Error(localizeUi('현재 원고는 Markdown 원문 모드입니다. Word 작업은 새 Word 편집본에서 시작하세요.'));
  ...
  private async refreshTemplateChoices():Promise<void>{
    if(!this.quickTemplate)return;const selected=this.project.preset.template?.id??'';
    const templates=await this.library().list();if(this.closed||!this.quickTemplate)return;
    const signature=JSON.stringify(templates.map(t=>[t.id,t.name]));
    if(this.quickTemplate.dataset.signature!==signature){this.quickTemplate.empty();for(const template of templates)this.quickTemplate.createEl('option',{value:template.id,text:template.name});this.quickTemplate.dataset.signature=signature;}
    const resolved=await this.library().resolve(selected);this.quickTemplate.value=resolved?.id??selected;
  }
  private async applySavedTemplate(id:string):Promise<void>{
    await this.flushSourceDraft();
    const library=this.library(),template=await library.resolve(id);if(!template)throw Error(localizeUi('저장된 템플릿을 찾을 수 없습니다.'));
    const next=cloneJournal(this.project),projectId=next.id,revision=next.revision;
    await copyTemplateAssets(template,library.store,this.store);applyTemplate(next,template);
    const missing=await ensureProjectFonts(next,this.store);if(missing.length){await this.refreshTemplateChoices();throw Error(localizeUi('템플릿을 적용하지 않았습니다. 필요한 글꼴: ')+missing.join(', '));}
    if(this.project.id!==projectId||this.project.revision!==revision)throw Error(localizeUi('원고가 변경됐습니다. 템플릿을 다시 선택하세요.'));
    if(this.project.markdown){
      this.sourceEditAt=0;await this.changeFields({'aaeu-template':id});await this.flushSourceDraft();
      // Reimport applies the same saved template, including its portable assets.
      this.project.markdown.templateId=undefined;
      const file=this.app.vault.getAbstractFileByPath(this.project.markdown.path);if(!(file instanceof TFile))throw Error(localizeUi('연결된 원문을 찾을 수 없습니다.'));
      await this.importSourceNote(file,this.sourceDraft!,true);
    }else this.edit(p=>{p.preset=next.preset;p.fonts=next.fonts;p.assets=next.assets;});
    await this.save();await this.compose();
  }
  private async checkTemplateMismatch(text:string):Promise<void>{
    let id:unknown;try{id=sourceProperties(text)['aaeu-template'];}catch{return;}
    const current=this.project.preset.template;if(typeof id!=='string'||!current||id===current.id)return;
    const library=this.library(),sourceTemplate=await library.resolve(id);if(sourceTemplate&&sourceTemplate.id===(await library.resolve(current.id))?.id)return;
    const modal=new Modal(this.app);modal.titleEl.setText(localizeUi('원문과 프로젝트의 템플릿이 다릅니다'));
    modal.contentEl.createEl('p',{text:localizeUi("원문: {v0} / 현재 지면: {v1}. 사용할 템플릿을 고르면 원문과 프로젝트를 함께 맞춥니다.",{v0:id,v1:current.name})});
    action(modal.contentEl,localizeUi('원문에 지정된 템플릿 사용'),()=>this.run(async()=>{modal.close();await this.applySavedTemplate(id);}));
    action(modal.contentEl,localizeUi('현재 지면 템플릿 사용'),()=>this.run(async()=>{modal.close();await this.applySavedTemplate(current.id);}));modal.open();
  }
  private openTemplates():void{
    const id=this.project.id,revision=this.project.revision;
    new JournalTemplateWizard(this.app,this.gateway,this.project,this.store,this.library(),next=>{
      if(this.project.id!==id||this.project.revision!==revision){new Notice(localizeUi('원고가 변경됐습니다. 저장된 템플릿을 선택해 적용하세요.'));return;}
      void this.run(()=>this.applySavedTemplate(next.preset.template!.id));
    }).open();
  }
  ...
  private selectNode(id:string,navigate=true):void{
    this.selected=id;
    const key=fieldForSelection(this.project,id);if(navigate){if(key)this.manuscript?.focusProperty(key);else if(this.project.markdown&&this.sourceDraft!==null)this.manuscript?.focusLine(sourceLine(this.project,id,this.sourceDraft));else if(!this.project.markdown)this.editor?.focusNode(id);}
    ...
    this.renderInspector();if(this.project.markdown)action(this.inspector,t('원문 위치로 이동'),()=>this.run(()=>this.openMarkdownSource(id)));if(navigate)this.preview?.focusNode(id,this.result?.boxes??[]);
  ...
  async importMarkdownNote(file:TFile,text:string,reimport=false):Promise<void>{
    ...
    if(reimport&&this.project.editorial?.changes.some(c=>c.status==='pending'))throw new Error(localizeUi('이전 재가져오기의 차이를 검토한 뒤 다시 가져오세요.'));
    ...
    const next=createPublicJournalProject();next.preset=cloneJournal(this.project.preset);next.fonts=cloneJournal(this.project.fonts);
  ...
  async createMarkdownManuscript():Promise<void>{
    ...
    const file=await this.app.vault.create(target,manuscriptTemplate(selected,uiLanguage()));
    await this.importSourceNote(file,await this.app.vault.read(file),false);this.manuscript?.focusProperty('aaeu-title');
  }
  async openSampleManuscript(language:SampleLanguage=uiLanguage(),newCopy=false):Promise<void>{
    const path='Academic Editor Ultra Samples/'+(language==='ko'?localizeUi('한국어'):'English')+'/Start here.md';
    ...
    this.manuscript?.focusProperty('aaeu-title');
  }
  private async importSourceNote(file:TFile,text:string,reimport:boolean,refreshRemote=false):Promise<void>{
    this.ocrController?.abort();await this.save();
    const previous=this.project,previousId=previous.id,previousRevision=previous.revision,next=createPublicJournalProject();next.preset=cloneJournal(previous.preset);next.fonts=cloneJournal(previous.fonts);
    const store=reimport?this.store:new JournalStore(this.app.vault.adapter,PROJECTS+'/'+next.id);
    ...
    const assetQueue=[next.preset.master?.logoAssetId,next.preset.master?.crossmarkAssetId];for(const id of assetQueue){const a=previous.assets.find(a=>a.id===id);if(a&&!next.assets.some(v=>v.id===a.id)){next.assets.push(cloneJournal(a));if(a.derivedFrom)assetQueue.push(a.derivedFrom);if(!reimport){const bytes=await this.store.get(a.path);if(bytes)await store.put(a.path,bytes);}}}
    const metadata=sourceProperties(text),templateId=typeof metadata['aaeu-template']==='string'?metadata['aaeu-template']:undefined;
    if(templateId){const library=this.library(),template=await library.resolve(templateId);if(template){if(!reimport||previous.markdown?.templateId!==templateId){await copyTemplateAssets(template,library.store,store);applyTemplate(next,template);}}else next.issues.push({id:'property:aaeu-template',code:'markdown-property',severity:'error',nodeId:'property:aaeu-template',message:t('aaeu-template: 등록된 템플릿을 찾을 수 없습니다: ')+templateId});}
    ...
    this.store=store;this.history=new JournalHistory(result);if(!reimport){this.result=null;this.preview?.clear();}this.sourceStale=false;this.refresh();await this.save();
  }
  ...
  private async refreshSourceNow(refreshRemote:boolean):Promise<void>{
    ...
    const file=this.app.vault.getAbstractFileByPath(binding.path);if(!(file instanceof TFile))throw new Error(localizeUi('연결된 Markdown 원문이 없습니다. 원문 다시 연결을 사용하세요: ')+binding.path);
    ...
    if(changed||refreshRemote){await this.importSourceNote(file,text,true,refreshRemote);if(this.sourceDraft===this.sourceBase){this.sourceBase=this.sourceDraft=text;this.manuscript?.setSource(text);}}else this.sourceStale=false;
  }
  private async assertSourceFresh():Promise<void>{
    const result=this.result;await this.refreshBoundSource();if(result!==this.result||this.sourceStale||result?.fingerprint!==await digestBytes(jsonBytes(this.project)))throw new Error(t('내보내는 동안 원문이 변경됐습니다. 새 미리보기를 확인하세요.'));
  }
  ...
  private showPropertyGuide():void{
    ...
    const keys=this.project.markdown?.properties??{};const area=modal.contentEl.createEl('textarea');area.rows=18;area.value=propertyTemplate(this.project.preset.template?.id,uiLanguage());area.readOnly=true;area.setAttribute('aria-label',t('전체 YAML 속성 템플릿'));
    for(const spec of MARKDOWN_PROPERTIES)modal.contentEl.createEl('p',{text:spec.key+' · '+localizedProperty(spec,uiLanguage()).label+(keys[spec.key]!==undefined?t(' · 입력됨'):'')});modal.open();
  }
  private async transitionMarkdown():Promise<void>{
    ...
    if(!(file instanceof TFile))throw new Error(localizeUi('이전 Markdown 원문이 없습니다. Markdown 가져오기로 원문을 선택하세요. 기존 프로젝트는 그대로 보존됩니다.'));
    const text=await this.sourceText(file),modal=new Modal(this.app);modal.titleEl.setText(localizeUi('원문 모드로 별도 복사 · 기존 편집본 보존'));
    modal.contentEl.createEl('p',{text:localizeUi('왼쪽 편집본의 수동 수정은 자동으로 Markdown에 합치지 않습니다. 아래 편집본과 원문을 대조하여 필요한 수정을 원문에 먼저 반영하세요.')});
    for(const [label,value]of [[localizeUi('기존 편집본'),JSON.stringify(this.project.document,null,2)],[localizeUi('현재 Markdown 원문'),text]]){modal.contentEl.createEl('h3',{text:label});const area=modal.contentEl.createEl('textarea');area.value=value;area.readOnly=true;area.rows=10;area.setAttribute('aria-label',label);}
    action(modal.contentEl,localizeUi('차이를 확인했습니다 · 원문으로 새 프로젝트 복사'),()=>this.run(async()=>{await this.importSourceNote(file,await this.sourceText(file),false);modal.close();}));
    action(modal.contentEl,t('취소'),()=>modal.close());modal.open();
  }
  private sourceInspector(host:HTMLElement):void{
    host.createEl('h3',{text:t('원문 속성·내용 확인')});host.createEl('p',{text:t('왼쪽에서 수정하면 연결된 Markdown 원문에도 저장됩니다. 템플릿·배치 설정과 확인 기록은 프로젝트에 보관됩니다.')});
    ...
    if(this.panel==='references'){
      ...
      for(const key of ['aaeu-title','aaeu-abstract','aaeu-running-title','aaeu-received','aaeu-revised','aaeu-accepted','aaeu-corresponding-name','aaeu-data-text','aaeu-conflict-text','aaeu-acknowledgments-text'])action(host,(MARKDOWN_PROPERTIES.map(s=>localizedProperty(s,uiLanguage())).find(s=>s.key===key)?.label??key),()=>this.manuscript?.focusProperty(key));
    }
  ...
  async migrateHanmark():Promise<void>{
    ...
    if(!await this.app.vault.adapter.exists('HanMark Journals'))throw new Error(localizeUi('이 볼트에 HanMark Journals 폴더가 없습니다. ZIP 열기도 사용할 수 있습니다.'));
    const modal=new Modal(this.app);modal.titleEl.setText(localizeUi('HanMark 프로젝트 복사 · 원본 보존'));
    for(const root of (await this.app.vault.adapter.list('HanMark Journals')).folders){try{const p=await new JournalStore(this.app.vault.adapter,root).load();action(modal.contentEl,p.document.title||root,()=>this.run(async()=>{await this.save();const copied=await copyLegacyProject(this.app.vault.adapter,root,PROJECTS);this.history=new JournalHistory(copied.project);this.store=copied.store;this.result=null;this.preview?.clear();this.refresh();modal.close();this.status.setText(copied.existing?localizeUi('이미 복사한 프로젝트를 열었습니다.'):localizeUi('복사 완료 · 기존 편집 방식 유지 · 원본 변경 없음'));}));}catch{/* Not a project. */}}modal.open();
  }
  private async scanCaptions(onlyId?:string):Promise<void>{
    ...
    try{
      ...
      for(const asset of assets){
        ...
        try{detection=await this.ocr.detect(asset,bytes,controller.signal,message=>this.status.setText(t(message)));}
  ...
  private async composeLatest():Promise<void>{
    ...
    await restoreLegacyTemplateAssets(this.project,this.library(),this.store);
    ...
    try{
      do{
        ...
        if(this.project.id!==projectId||this.project.revision!==revision){this.markPreviewDirty();if(!this.autoPreview)return;this.composeAgain=true;continue;}
        ...
        const result=await this.composer.compose(snapshot,this.store,controller.signal,message=>this.status.setText(t(message)),true);
        if(this.closed||controller.signal.aborted)return;
        await this.refreshBoundSource();
        if(result.fingerprint!==await digestBytes(jsonBytes(this.project))){this.markPreviewDirty();if(!this.autoPreview)return;this.composeAgain=true;continue;}
        this.result=result;await this.preview?.show(result);this.previewDirty=false;this.previewState?.setText(uiLanguage()==='ko'?localizeUi('최신 미리보기'):'Preview up to date');this.manuscript?.update(this.project,result);
        if(this.closed||controller.signal.aborted)return;
        this.status.setText(uiLanguage()==='ko'?localizeUi("{v0}쪽 · {v1}초 · 검사 {v2}건",{v0:result.pageCount,v1:(result.elapsedMs/1000).toFixed(1),v2:result.issues.length}):`${result.pageCount} pages · ${(result.elapsedMs/1000).toFixed(1)} s · ${result.issues.length} checks`);this.renderInspector();
  ...
  private async exportAf():Promise<void>{
    ...
    try{
      ...
      const output=await journalAfExport(project,store,snapshot,result.pdf,{...options,columns,signal:controller.signal,progress:message=>{current();this.status.setText(t(message));}});
  ...
  private async exportPdf(final:boolean):Promise<void>{
    ...
    if(!result||result.fingerprint!==await digestBytes(jsonBytes(this.project)))throw Error(localizeUi('현재 원고의 PDF를 생성한 뒤 저장하세요.'));
    const remaining=exportReadiness(this.project,result);
    if(final&&remaining.length){
      const approved=await new Promise<boolean>(resolve=>{
        const modal=new Modal(this.app);modal.titleEl.setText(localizeUi('아직 확인하지 않은 항목이 있습니다'));let accepted=false;
        modal.contentEl.createEl('p',{text:localizeUi('다음 항목이 해결되지 않았습니다. 빈 정보는 인쇄하지 않습니다. 그래도 현재 PDF를 저장할 수 있습니다.')});
        const list=modal.contentEl.createEl('ul');for(const issue of remaining)list.createEl('li',{text:t(issue.message)});
        action(modal.contentEl,localizeUi('돌아가 수정'),()=>modal.close());action(modal.contentEl,localizeUi('그래도 출력'),()=>{accepted=true;modal.close();});
        modal.onClose=()=>resolve(accepted);modal.open();
      });
      if(!approved)return;
      await this.assertSourceFresh();if(this.result!==result||result.fingerprint!==await digestBytes(jsonBytes(this.project)))throw Error(localizeUi('확인 중 원고가 변경됐습니다. 최신 지면을 다시 확인하세요.'));
    }
    await this.save();await this.assertSourceFresh();
    const saved=await this.gateway.saveFile(result.pdf,this.filename()+(final?'':'_review')+'.pdf');
    if(!saved.cancelled&&final&&remaining.length)await this.store.put(`export-receipts/${Date.now()}.json`,jsonBytes({version:1,at:new Date().toISOString(),revision:this.project.revision,fingerprint:result.fingerprint,sourceSha256:this.project.markdown?.sha256,override:true,issues:remaining}));
  }
  private placementUndo:{before:LayoutOverride[];after:LayoutOverride[];source:string|null}[]=[];
  private placementRedo:typeof this.placementUndo=[];
  private async applyOverride(o:LayoutOverride):Promise<void>{
    if(this.busy)throw Error(t('조판을 마친 뒤 배치를 변경하세요.'));
    const before=cloneJournal(this.project.overrides);
    this.edit(p=>{p.overrides=p.overrides.filter(v=>v.id!==o.id);p.overrides.push(o);},false);
    try{
      await this.compose();
      if(this.result?.issues.some(i=>i.nodeId===o.id&&['outside-body','overlap','figure-caption-space','table-pin-too-small','figure-too-tall'].includes(i.code)))throw Error(t('배치할 공간이 부족합니다'));
    }catch(error){this.edit(p=>{p.overrides=before;},false);await this.compose();throw error;}
    this.placementUndo.push({before,after:cloneJournal(this.project.overrides),source:this.sourceDraft});this.placementRedo=[];
    await this.save();this.renderInspector();
  }
  private undoPlacement(redo=false):boolean{
    const from=redo?this.placementRedo:this.placementUndo,to=redo?this.placementUndo:this.placementRedo,item=from.at(-1);
    if(!item||item.source!==this.sourceDraft||JSON.stringify(this.project.overrides)!==JSON.stringify(redo?item.before:item.after))return false;
    from.pop();to.push(item);this.edit(p=>{p.overrides=cloneJournal(redo?item.after:item.before);},false);void this.run(async()=>{await this.save();await this.compose();});return true;
  }
  private renderInspector():void{
    ...
    if(p.sources.length||d.blocks.length){
      const missingTitle=!d.title.trim(),missingAuthor=academicChecks(p)&&!d.journalMetadata?.correspondence&&!d.authors.some(a=>a.corresponding&&a.name.trim()&&a.email?.trim());
      ...
      if(missingTitle||missingAuthor||figures.length){
        ...
        for(const f of figures)if(f.kind==='figure')action(card,localizeUi("{v0} 원본 연결",{v0:f.caption?.number?'Figure '+f.caption.number:t('그림')}),()=>{this.selected=f.id;this.panel='objects';this.renderInspector();void this.run(()=>this.replaceFigure(f.id));});
        card.createEl('p',{text:t('빈 정보와 표제는 출력하지 않습니다. 누락된 그림은 교체 자리와 경고를 남깁니다.')});
      }
    }
    if(['article','publication','endMatter'].includes(this.panel)){host.createEl('p',{text:t('왼쪽 원고 편집기에서 정보를 수정하세요. 미리보기의 해당 영역을 클릭하면 입력란으로 이동합니다.')});action(host,t('해당 입력란으로 이동'),()=>this.manuscript?.focusProperty(fieldForSelection(p,this.selected)??(this.panel==='endMatter'?'aaeu-data-text':this.panel==='publication'?'aaeu-year':'aaeu-title')));if(p.markdown)this.sourceInspector(host);return;}
    ...
    if(this.panel==="publication")this.publicationPanel(host);
      ...
      field(host,localizeUi("저자 · 한 줄에 이름 | 소속 번호 | 이메일"),d.authors.map(a=>[a.name,a.affiliations.join(","),a.email??""].join(" | ")).join("\n"),v=>this.edit(p=>{p.document.authors=v.split("\n").filter(Boolean).map((line,index)=>{const [name,aff,email]=line.split("|").map(s=>s.trim());const old=p.document.authors.find(a=>a.name===name)??p.document.authors[index];return {...old,name,affiliations:aff?.split(",")??[],email,corresponding:!!email};});}),true);
      for(const [i,a]of d.authors.entries())if(a.corresponding)field(host,localizeUi("{v0} 교신저자 주소 · 줄바꿈 유지",{v0:a.name}),a.address??"",v=>this.edit(p=>{p.document.authors[i].address=v;},false),true);
      field(host,localizeUi("소속 · 한 줄에 하나"),d.affiliations.join("\n"),v=>this.edit(p=>{p.document.affiliations=v.split("\n");},false),true);
      field(host,localizeUi("소속 위첨자 · 한 줄에 하나 (예: 1,2)"),d.affiliations.map((_,i)=>d.affiliationMarkers?.[i]??String(i+1)).join("\n"),v=>this.edit(p=>{p.document.affiliationMarkers=v.split("\n");},false),true);
      ...
      field(host,localizeUi("키워드 · 세미콜론 구분"),d.keywords.join("; "),v=>this.edit(p=>{p.document.keywords=v.split(";").map(s=>s.trim()).filter(Boolean);},false));
      host.createEl("p",{text:localizeUi("원본 {v0}개 · 그림 {v1}개 · 변경 기록 {v2}개",{v0:p.sources.length,v1:p.assets.length,v2:p.changes.length})});
      if(d.importedMetadata?.length){
        const details=host.createEl("details");details.createEl("summary",{text:localizeUi("원고 정보 추출 근거 · 원문 보존")});
      ...
      if(!referenceChecks(p))host.createEl("p",{text:localizeUi("참고문헌 검사가 꺼져 있습니다. 내용과 출력 서식은 유지되며, 온라인 조회는 아래 버튼으로 실행합니다.")});
      ...
      for(const ref of p.references){const row=host.createDiv({cls:"aaeu-journal-card"});row.dataset.referenceId=ref.id;row.createEl("p",{text:(ref.confirmed?"✓ ":t("미확정 · "))+(ref.title||ref.raw||t("제목 없음"))});action(row,t("편집"),()=>this.run(async()=>{const r=await referenceDialog(this.app,ref);if(r)this.edit(p=>{p.references=p.references.map(v=>v.id===r.id?r:v);});}));
        field(row,localizeUi('저자/단체명 정렬 키'),ref.sort?.authorKey??inferredAuthorKey(ref)??'',v=>this.edit(p=>{const r=p.references.find(r=>r.id===ref.id)!;r.sort={...r.sort,authorKey:v,confirmed:!!v.trim()};},false));
        field(row,localizeUi('동일 저자·연도의 제목 정렬 키'),ref.sort?.titleKey??ref.title,v=>this.edit(p=>{const r=p.references.find(r=>r.id===ref.id)!;r.sort={authorKey:r.sort?.authorKey??inferredAuthorKey(r)??'',confirmed:r.sort?.confirmed??false,titleKey:v};},false));
      }
    }else if(this.panel==="changes"){
      host.createEl("p",{text:localizeUi("원고 창의 색 표시에는 변경 전·후 텍스트가 모두 보입니다. PDF는 현재 결정에 따른 최종 보기를 사용합니다.")});
      ...
      host.createEl("p",{text:localizeUi("저장된 글꼴: {v0}",{v0:[...new Set(p.fonts.map(f=>f.family))].join(", ")||t("조판 시 자동으로 가져옵니다.")})});
      ...
      if(this.result?.numbering?.length){const numbers=host.createEl('details');numbers.createEl('summary',{text:t('원고 번호 → 출력 번호')});for(const n of this.result.numbering)action(numbers,localizeUi("{v0} {v1} → {v2} · {v3}쪽",{v0:n.kind,v1:n.original||t('없음'),v2:n.number,v3:n.page}),()=>this.selectNode(n.id));}
      host.createEl("p",{text:localizeUi("검사 {v0}건 · 미결정 변경 {v1}건",{v0:issues.length,v1:p.changes.filter(c=>c.decision==="pending").length})});
      if(this.result?.coverage)host.createEl('p',{text:localizeUi("출력 내용 대조: {v0} · {v1}개 항목",{v0:this.result.coverage.complete?t('모두 대응'):t('미해결 있음'),v1:this.result.coverage.entries.length})});
      const ledger=host.createEl('details');ledger.createEl('summary',{text:localizeUi("자동 보정 기록 {v0}건",{v0:this.result?.adjustments?.length??0})});
      for(const a of this.result?.adjustments??[]){const row=ledger.createDiv({cls:'aaeu-journal-card'});row.createEl('p',{text:`${a.rule}: ${a.before} → ${a.after} · ${a.reason}`});action(row,t('항목 선택'),()=>{this.selected=a.nodeId;this.panel='objects';this.renderInspector();});action(row,localizeUi('이 항목 자동 보정 해제'),()=>this.edit(p=>{const q=compositionQuality(p);p.preset.compositionQuality={...q,disabledNodes:[...new Set([...q.disabledNodes,a.nodeId])]};}));}
      for(const i of issues){const row=host.createDiv({cls:"aaeu-journal-card"});row.createEl("p",{text:`${i.severity} · ${t(i.message)}`});
  ...
  private publicationPanel(host:HTMLElement):void{
    ...
    host.createEl("p",{text:localizeUi("첫 페이지의 영역을 클릭해 값을 수정하세요. 글꼴·색·위치는 저널 프리셋이 적용합니다.")});
    ...
    for(const [key,label]of [["year",t("발행 연도")],["volume",t("권")],["issue",t("호")],["doi",localizeUi("DOI · doi.org 주소도 입력 가능")]] as const)field(issue,label,d[key],v=>this.edit(p=>{p.document[key]=key==="doi"?canonicalDoi(v):v.trim();},false));
    ...
    issue.createEl("p",{text:pages?localizeUi("최근 출력 {v0}쪽 · 확정본 {v1}–{v2}쪽 · 다음 논문 시작 {v3}쪽",{v0:pages,v1:d.firstPage,v2:d.firstPage+pages-1,v3:d.firstPage+pages}):localizeUi("조판 후 끝 쪽수와 다음 논문의 시작 쪽수를 자동 계산합니다.")});
    if(publicationMode(d)==="aop")issue.createEl("p",{text:localizeUi("입력한 권·호·시작 쪽수는 보관되며 AOP PDF에는 표시되지 않습니다.")});
    const side=section("correspondence",t("심사 날짜와 교신저자"));
    for(const [key,label]of [["received",localizeUi("Received · 접수일")],["revised",localizeUi("Revised · 수정일")],["accepted",localizeUi("Accepted · 승인일")]] as const)field(side,label,d[key],v=>this.edit(p=>{p.document[key]=v;},false));
    for(const [i,a]of d.authors.entries()){
      const card=side.createDiv({cls:"aaeu-journal-card"});
      choose(card,localizeUi("{v0} 교신저자",{v0:a.name||"저자 "+(i+1)}),a.corresponding?"yes":"no",{yes:t("교신저자"),no:localizeUi("일반 저자")},v=>this.edit(p=>{p.document.authors[i].corresponding=v==="yes";}));
      if(a.corresponding)for(const [key,label]of [["name",t("이름")],["email",t("이메일")],["address",t("주소")]] as const)field(card,localizeUi("교신저자 {v0} {v1}",{v0:i+1,v1:label}),a[key]??"",v=>this.edit(p=>{p.document.authors[i][key]=v;},false),key==="address");
    }
    ...
    field(copyright,localizeUi("판권 연도 · 비우면 발행 연도 사용"),d.publication?.copyrightYear??"",v=>this.edit(p=>{p.document.publication={...p.document.publication,mode:publicationMode(p.document),copyrightYear:v.trim()};},false));
    copyright.createEl("p",{text:this.project.preset.appearance?.copyrightText!==undefined?localizeUi("표시: ")+(resolveTemplateText(this.project.preset.appearance.copyrightText,this.project)||t("숨김")):localizeUi("표시: Copyright © {v0} {v1}",{v0:copyrightYear(d)||t("연도 미입력"),v1:m.copyrightOwner})});
    ...
    action(host,t("현재 발행정보 확인·확정"),()=>this.run(async()=>{
      if(!this.result||this.result.fingerprint!==await digestBytes(jsonBytes(this.project))){await this.compose();this.status.setText(localizeUi("미리보기를 갱신했습니다. 표시된 발행정보를 확인한 뒤 다시 확정하세요."));return;}
  ...
  private objects(host:HTMLElement):void{
    action(host,localizeUi("텍스트 데이터로 표 만들기"),()=>this.run(()=>this.addData(false)));
    action(host,localizeUi("데이터로 차트 만들기"),()=>this.run(()=>this.addData(true)));
    ...
    for(const b of this.result?.boxes.filter(b=>b.kind==="text-frame")??[])choices[b.nodeId]=localizeUi("{v0}쪽 텍스트 프레임 {v1}",{v0:b.page,v1:b.nodeId.endsWith(":0")?t("왼쪽"):t("오른쪽")});
    ...
    if(node){
      if(node.kind==='anchor'){
        host.createEl('p',{text:localizeUi("원문 삽입 지점: {v0}",{v0:inlineText(node.source.content)})});
      ...
      const q=compositionQuality(this.project);choose(host,localizeUi('이 항목 자동 보정'),q.disabledNodes.includes(node.id)?'no':'yes',{yes:t('프리셋 기준 사용'),no:t('사용 안 함')},v=>this.edit(p=>{const q=compositionQuality(p);p.preset.compositionQuality={...q,disabledNodes:v==='yes'?q.disabledNodes.filter(id=>id!==node.id):[...new Set([...q.disabledNodes,node.id])]};}));
      ...
      if(node.kind==="paragraph"||node.kind==="heading"){
        for(const mention of objectMentions(this.project).filter(m=>m.nodeId===node.id)){
          ...
          for(const [i,number]of mention.numbers.entries())choose(host,localizeUi("{v0}: {v1} 대상",{v0:mention.text,v1:number}),selected[i],{'':t('대상 선택'),...choices},v=>{selected[i]=v;});
          action(host,localizeUi('이 본문 참조 연결'),()=>{if(selected.every(Boolean))this.edit(p=>bindMention(p,mention,selected));});
        }
        choose(host,localizeUi("용도"),node.role??"body",{body:t("본문"),reference:localizeUi("원문 참고문헌"),quote:localizeUi("인용"),note:localizeUi("주")},v=>this.edit(p=>{const n=p.document.blocks.find(n=>n.id===node.id) as Paragraph;n.role=v as Paragraph["role"];}));
        action(host,localizeUi("제목으로 지정하고 본문에서 제거"),()=>this.edit(p=>{p.document.title=inlineText(node.content);p.document.importedMetadata??=[];p.document.importedMetadata.push({field:"title",sourceId:node.origin?.sourceId??"",blocks:[cloneJournal(node)],rule:"editor-selection"});p.document.blocks=p.document.blocks.filter(n=>n.id!==node.id);}));
      }
      if(node.kind==="figure"||node.kind==="table"){
        if(node.kind==='figure'){
          ...
          if(node.assetId){
            ...
            const detection=this.project.editorial?.detections.find(d=>d.assetId===node.assetId);if(detection)host.createEl('p',{text:detection.status==='failed'?localizeUi("OCR 확인 필요: {v0}",{v0:detection.message}):localizeUi("내장 캡션 후보 {v0}개{v1}",{v0:detection.candidates.length,v1:node.crop?.confirmed?' · 크롭 적용됨':''})});
          }
        }
        if(node.kind==="table"&&node.normalization){
          host.createEl("p",{text:node.normalization.confirmed?localizeUi("통계표 행·열 대응 확인 완료"):localizeUi("한 셀의 여러 통계값을 개별 행으로 정리했습니다. 아래 원본과 비교해 행·열 대응을 확인하세요.")});
          const details=host.createEl("details");details.createEl("summary",{text:localizeUi("정리 전 셀과 문단 보기")});
          ...
          action(host,localizeUi("표 행·열 대응 확인·확정"),()=>this.edit(p=>{const n=p.document.blocks.find(b=>b.id===node.id);if(n?.kind==="table"&&n.normalization)n.normalization.confirmed=true;}));
          action(host,localizeUi("정리 전 표 구조로 복원"),()=>this.edit(p=>{const n=p.document.blocks.find(b=>b.id===node.id);if(n?.kind==="table"&&n.normalization){n.rows=cloneJournal(n.normalization.sourceRows);n.columnWeights=[...n.normalization.sourceColumnWeights];delete n.normalization;}}));
        }
        if(node.kind==='table'&&embeddedTableCaption(node)&&node.caption?.number){
          host.createEl('p',{text:localizeUi('표 안팎의 제목 중 사용할 제목을 선택하세요. 중복 제목 행만 제거합니다.')});
          host.createEl('pre',{text:localizeUi('표 안: ')+inlineText(embeddedTableCaption(node)!.content)+localizeUi('\n표 밖: ')+inlineText(node.caption.title)});
          action(host,localizeUi('표 안 제목 사용'),()=>this.edit(p=>resolveEmbeddedTableCaption(p,node.id,'inside')));
          action(host,localizeUi('표 밖 제목 사용'),()=>this.edit(p=>resolveEmbeddedTableCaption(p,node.id,'outside')));
        }
        ...
        field(host,localizeUi("주"),node.caption?.notes.map(b=>inlineText(b.content)).join("\n")??"",v=>this.edit(p=>{const n=p.document.blocks.find(n=>n.id===node.id) as typeof node;n.caption??={number:"",title:[],notes:[]};n.caption.notes=v.split("\n").filter(Boolean).map(text=>({id:newId("note"),kind:"paragraph",role:"note",content:[{text}]}));},false),true);
      }
    }
    const box=this.result?.boxes.find(b=>b.nodeId===this.selected),pin=this.project.overrides.find(o=>o.id===this.selected);
    if(box&&['figure','table'].includes(box.kind)){
      this.preview?.selectObject(box);
      for(const [lane,label]of [['left',localizeUi('왼쪽 칼럼')],['right',localizeUi('오른쪽 칼럼')],['full',localizeUi('전체 폭')]] as const)action(host,t(label),()=>this.preview?.setLane(lane));
      action(host,t('자동 배치로 복원'),()=>this.run(async()=>{this.edit(p=>{p.overrides=p.overrides.filter(o=>o.id!==box.nodeId);},false);await this.compose();}));
    }
    if(box&&box.kind==='text-frame'){
  ...
  private async addData(chart:boolean):Promise<void>{
    const raw=await textDialog(this.app,chart?localizeUi("차트 데이터"):localizeUi("표 데이터"),localizeUi("첫 행은 열 제목입니다. 엑셀의 셀을 복사해 탭으로 구분된 텍스트를 붙여 넣으세요."),"",true);if(!raw)return;
    const rows=raw.trim().split(/\r?\n/).map(line=>line.split("\t"));if(rows.length<2)throw new Error(localizeUi("머리행과 데이터 행이 필요합니다."));
    ...
    if(chart){
      ...
      if(!series.length||rows.slice(1).some(r=>r.length!==rows[0].length||r.slice(1).some(v=>!v.trim()||!Number.isFinite(Number(v)))))throw new Error(localizeUi("첫 열은 분류명, 나머지 열은 빈칸 없는 숫자여야 합니다."));
  ...
  async onClose():Promise<void>{try{await this.flushSourceDraft();}catch(e){if(this.sourceDraft!==null)await this.store.put(`recovery/draft-${Date.now()}.md`,new TextEncoder().encode(this.sourceDraft));this.report(e);}this.closed=true;if(this.draftTimer)this.contentEl.win.clearTimeout(this.draftTimer);this.manuscript?.destroy();this.manuscript=null;this.manuscriptKey="";if(this.saveTimer)this.contentEl.win.clearTimeout(this.saveTimer);if(this.renderTimer)this.contentEl.win.clearTimeout(this.renderTimer);this.controller?.abort();this.afController?.abort();this.ocrController?.abort();this.ocr.cancel();this.engine.dispose();this.editor?.destroy();this.editor=null;this.editorKey="";this.preview?.destroy();if(this.dirty)await this.run(()=>this.save());}
}
