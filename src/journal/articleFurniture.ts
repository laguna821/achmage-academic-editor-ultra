import {appearanceFromPreset,academicChecks} from './appearance';
import {MISSING_CORRESPONDENCE} from './missingContent';
...
export function sidebarItems(p:JournalProject):SidebarItem[]{
  ...
  const corr:SidebarItem[]=(correspondence||academicChecks(p))?[{id:'correspondence',label:'Corresponding author:',text:correspondence||(academicChecks(p)?MISSING_CORRESPONDENCE:''),separate:true}]:[];
  const custom=(m?.sidebar??[]).map(s=>({...s,separate:true}));
